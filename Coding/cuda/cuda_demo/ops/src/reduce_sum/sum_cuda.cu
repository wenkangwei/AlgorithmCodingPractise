...
// warp reduce sum, only for power of 2 block size, using shuffle instruction, no shared memory is used
//ref: https://zhuanlan.zhihu.com/p/572901115
template <typename T>
...
__global__ void sum_reduce_kernel(const float* array, float* ans, int n){
    /*
     ...
     - The kernel uses warp-level reduction to compute the sum of elements in each warp.
     - The results from each warp are stored in shared memory and then reduced further.
     - The final result is atomically added to the output variable.
    */
